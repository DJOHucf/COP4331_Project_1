...
function addContactMultiField() {
	...
	// ENHANCED VALIDATION: Check for duplicates (name, email, phone)
...
function createContactRecord(firstName, lastName, emailAddress, phoneNumber) {
    // Send field names that match your PHP/database expectations
    let tmp = {
        firstName: firstName,    // Keep as lowercase - your PHP expects this
        lastName: lastName,      // Keep as lowercase - your PHP expects this
        phone: phoneNumber,      // Changed from 'phone' to match PHP
        email: emailAddress,     // Keep as lowercase - your PHP expects this
        userID: userId           // Changed to match SearchContacts.php expectation
    };

    let jsonPayload = JSON.stringify(tmp);
    console.log("Sending payload:", jsonPayload); // Debug
    ...
    try {
        xhr.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                console.log("Server response:", xhr.responseText); // Debug
                ...
                if (jsonObject.error === "" || jsonObject.error === null) {
                    ...
                    // Instead of manually adding to table, refresh from database
                    // This ensures we get the correct ID and data consistency
...
function searchContact() {
    ...
    console.log("Searching with userId:", userId, "searchTerm:", searchTerm); // Debug
    ...
    // FIX: Send userId (lowercase) to match what PHP expects
    let tmp = {
        search: searchTerm,
        userId: userId  // Changed from userID to userId
    };
    let jsonPayload = JSON.stringify(tmp);
    console.log("Search payload:", jsonPayload); // Debug
    ...
    try {
        xhr.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                console.log("Search response:", xhr.responseText); // Debug
                ...
                let jsonObject = JSON.parse(xhr.responseText);
                console.log("Parsed search results:", jsonObject); // Debug

                if (tableBody) {
                    // New table structure
                    populateContactsTable(jsonObject.results || []);
                } else if (contactList) {
                    // Old contactList structure - keep existing code
                ...
                console.error("HTTP Error:", this.status, xhr.responseText); // Debug
                if (searchResultElement) {
        ...
        console.error("Search exception:", err); // Debug
        if (searchResultElement) {
...
function editContact(contactId) {
    // Find the button by contactId (capitalized ID from PHP)
    let button = document.querySelector(`button[onclick="editContact(${contactId})"]`);
    if (!button) {
        alert("Row not found for this contact!");
        return;
    }

    let row = button.closest("tr");

    // Grab current values from the row
    ...
    let newFirstName = prompt("Edit First Name:", firstName);
    let newLastName = prompt("Edit Last Name:", lastName);
    let newEmail = prompt("Edit Email:", email);
    let newPhone = prompt("Edit Phone:", phone);

    if (newFirstName === null || newLastName === null) return;

    // Build the update object (note capitalized "ID")
    let tmp = {
        ID: contactId,
...
function deleteContact(contactId) {
	console.log("Attempting to delete contact with ID:", contactId); // Debug
	...
	// Send ID (uppercase) to match what your PHP expects
	let tmp = {
		ID: contactId
	};
	
	let jsonPayload = JSON.stringify(tmp);
	console.log("Delete payload:", jsonPayload); // Debug
	...
	try {
		xhr.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				console.log("Delete response:", xhr.responseText); // Debug
